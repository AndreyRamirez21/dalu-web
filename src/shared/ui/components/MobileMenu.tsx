...
export function MobileMenu({ isOpen, onClose, links }: MobileMenuProps) {
  ...
  const openSectionsSet = useMemo(() => new Set(openSections), [openSections])
    ...
    >
      <aside className="absolute top-0 left-0 h-full w-72 bg-surface shadow-xl flex flex-col">
        ...
        <nav className="flex flex-col px-6 py-6 gap-1">
          {links.map((link) => {
            const isExpanded = openSectionsSet.has(link.to)
            return (
              <div key={link.to} className="border-b border-border">
                <div className="flex items-center justify-between">
                  ...
                  {link.children && <button type="button" aria-label={`Ver colecciones de ${link.label}`} aria-expanded={isExpanded} onClick={() => setOpenSections((current) => openSectionsSet.has(link.to) ? current.filter((section) => section !== link.to) : [...current, link.to])} className="p-2 text-text-secondary"><ChevronDown size={18} className={`transition-transform ${isExpanded ? 'rotate-180' : ''}`} /></button>}
                </div>
